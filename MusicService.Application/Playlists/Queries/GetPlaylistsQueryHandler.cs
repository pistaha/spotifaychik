...
{
    public class GetPlaylistsQueryHandler : IRequestHandler<GetPlaylistsQuery, List<PlaylistDto>>
    {
        private readonly IMusicServiceDbContext _dbContext;

        public GetPlaylistsQueryHandler(IMusicServiceDbContext dbContext)
        {
        ...
        public async Task<List<PlaylistDto>> Handle(GetPlaylistsQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                query = query.Where(p => p.CreatedById == request.UserId.Value);
            }
